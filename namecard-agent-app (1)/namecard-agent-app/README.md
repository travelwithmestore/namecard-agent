...
2. Add your API key to `.env`
3. Run `netlify dev` instead of `npm run dev` to test with functions locally
4. Never commit `.env` to git (it's in .gitignore)

### Important Security Note:

✅ **The API key is now secure!** It's stored on the server-side (Netlify Functions) and never exposed to the browser.
